        ...
        <div className="space-y-10">
          {sec.parts.map((part, i) => (
            <section key={i} id={`${sec.id}-p${i}`} className="scroll-mt-20">
              <h2 className="font-display mb-4 flex items-center gap-3 text-[26px] text-slate-100">
                <span className="section-number">{String(i + 1).padStart(2, '0')}</span>{part.title[lang]}
              </h2>
              <div className="space-y-4">
                {part.entries.map((e, j) => (
                  <article key={j} className="rounded-[var(--radius)] border border-border bg-card p-5 shadow-[var(--shadow-sm)]">
                    ...
                    <div className="mt-3 space-y-1 rounded-[var(--radius-sm)] bg-slate-900 px-3 py-2">
