...
export interface Hom {
  /** images of `gens`, the small generating set of G */
  imgs: number[]
  /** φ as a lookup: map[a] is the index in H of φ(a) */
  map: number[]
  kernel: number[]
  image: number[]
}
...
/**
 * Every homomorphism G → H (Herstein 2.5). A homomorphism is pinned down by what it does to a
 * generating set, so we try each tuple of candidate images and keep the ones that extend
 * consistently. Groups needing more than 3 generators are refused: the search is |H|^k.
 */
export function homomorphisms(g: Group, h: Group): Hom[] {
  const gens = smallGeneratingSet(g)
  if (gens.length > 3) return []
  const out: Hom[] = []
  const rec = (i: number, imgs: number[]) => {
    if (i === gens.length) {
      const map = extendHom(g, h, gens, imgs)
      if (map) out.push({ imgs: [...imgs], map, kernel: fiber(map, h.e), image: [...new Set(map)].sort((a, b) => a - b) })
      return
    }
    for (let y = 0; y < h.order; y++) rec(i + 1, [...imgs, y])
  }
  rec(0, [])
  return out
}

/** The elements of G that φ sends to `y`. The fibre over e′ is ker φ; the others are its cosets. */
export const fiber = (map: number[], y: number) => map.flatMap((v, a) => (v === y ? [a] : []))
